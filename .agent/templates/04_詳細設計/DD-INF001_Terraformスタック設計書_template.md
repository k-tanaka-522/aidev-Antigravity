# DD-INF001: Terraformスタック設計書 - {スタック名}
...
|------|------|
| **文書番号** | DD-INF001-{連番} |
| **文書名** | Terraformスタック設計書 - {スタック名} |
| **プロジェクト名** | {プロジェクト名} |
| **スタック名** | {スタック名}（例: network-vpc, security-sg, application-ecs） |
| **作成者** | {作成者名} |
| **作成日** | {YYYY/MM/DD} |
| **最終更新日** | {YYYY/MM/DD} |
...
**このスタックが管理するリソース**:
- {主要なリソースタイプ}（例: VPC, Subnet, Route Table, NAT Gateway）
...
**State保存先**:
- Backend: {backend_type}（例: S3 + DynamoDB）
- Bucket: `{bucket_name}`
- Key: `{state_key_path}`
- Lock Table: `{dynamodb_table_name}`

**State分離戦略**:
- {なぜこの単位でStateを分離したか}（例: 環境ごとに完全に独立させ、prod環境への影響を防ぐ）

**Workspace使用有無**:
- [ ] Workspace使用（理由: {理由}）
- [ ] Workspace不使用（理由: {環境ごとに別ディレクトリで管理するため}）

### 2.4 モジュール設計

**自作モジュール使用有無**:
- [ ] 自作モジュールを使用（モジュール名: `{module_name}`）
- [ ] 公式モジュールのみ使用

**モジュール化判断**:
- {なぜモジュール化したか／しなかったか}（例: 複数環境で同じVPC構成を再利用するため、vpc モジュールを作成）

**使用する外部モジュール**:

| モジュール名 | ソース | バージョン | 使用理由 |
|------------|--------|-----------|---------|
| {module_name} | {source_url} | {version} | {使用理由} |
...
**IAMロール/ポリシー**:
- {どのような権限設計にしたか}（例: Terraform実行用のIAMロールは最小権限の原則に従い、必要なリソースのみに絞る）

**推奨IAMポリシー**:
```hcl
# 参考: Terraform実行に必要な最小権限
{
  "Version": "2012-10-17",
  "Statement": [
    {
      "Effect": "Allow",
      "Action": ["ec2:Describe*", "ec2:CreateVpc", ...],
      "Resource": "*"
    }
  ]
}
...
|---------|---------|-------------|
| {リソース1} | {作成手順} | `{確認コマンド}` |
| State保存用S3バケット | 手動作成 | `aws s3 ls s3://{bucket}` |
...
| Output名 | 内容 | 参照するスタック |
|---------|------|----------------|
| `vpc_id` | VPC ID | security-sg, application-ecs |
| `private_subnet_ids` | プライベートサブネットIDリスト | application-ecs |
| {output名} | {内容} | {参照先} |
...
| Data Source | 取得内容 | 提供元スタック |
|------------|---------|--------------|
| `data.terraform_remote_state.{stack}` | {取得内容} | {提供元} |
...
1. **プレースホルダを置換**:
   - `{スタック名}`: 実際のスタック名（例: `network-vpc`, `security-sg`）
   - `{プロジェクト名}`: プロジェクト名
   - `{作成者名}`: 作成者名
   - `{YYYY/MM/DD}`: 日付
...
2. **不要なセクションは削除**:
   ...
   - 例: モジュールを使わない場合は「2.4 モジュール設計」を削除

3. **10-15ページに収める**:
   - パラメーター一覧は記載しない（コード参照）
   - 設計判断と理由のみ記載

4. **コードとの関係**:
   - この文書: 設計判断（Why）
   - Terraformコード: 実装（What）
   - **重複させない**
