...
> **重要**: 大規模システムでは、1つのテスト仕様書に全モジュールのテストを記載しないでください。
> クラス/モジュール単位で分割し、保守性を向上させてください。
...
1. **テストケース数が50を超える場合**
   - 📁 推奨: クラス単位、モジュール単位で分割
   - 例: `DD004_単体テスト仕様書_UserService.md`

2. **1ファイルが30ページを超える場合**
   - 📁 推奨: 機能単位で分割

3. **異なる担当者が実装する場合**
   - 📁 推奨: 担当者単位で分割し、並行作業を可能に
...
02_注文管理/
├── DD004_単体テスト仕様書_OrderService.md（40ケース）✅
└── DD004_単体テスト仕様書_OrderRepository.md（15ケース）✅
```
...
│   ├── DD004_単体テスト仕様書_AuthController.md
│   ├── DD004_単体テスト仕様書_UserRepository.md
│   └── DD004_単体テスト仕様書_PasswordHasher.md
│
├── 02_注文管理/
│   ├── DD004_単体テスト仕様書_OrderService.md
│   ├── DD004_単体テスト仕様書_OrderRepository.md
│   └── DD004_単体テスト仕様書_InventoryService.md
│
└── 03_共通基盤/
    ├── DD004_単体テスト仕様書_Logger.md
    └── DD004_単体テスト仕様書_ErrorHandler.md
```

### 命名規則

```
DD004_単体テスト仕様書_{クラス名/モジュール名}.md

例:
DD004_単体テスト仕様書_UserService.md
DD004_単体テスト仕様書_OrderController.md
DD004_単体テスト仕様書_PaymentGateway.md
```

### カバレッジ集計

複数ファイルに分割した場合、全体のカバレッジは以下のように集計してください：

```markdown
## 全体カバレッジ（サブシステム単位）

| モジュール | 行カバレッジ | 分岐カバレッジ | 関数カバレッジ | 状況 |
|-----------|-------------|---------------|---------------|------|
| UserService | 95% | 90% | 100% | ✅ |
| AuthController | 88% | 85% | 95% | ✅ |
| UserRepository | 92% | 88% | 100% | ✅ |
| **合計** | **91.7%** | **87.7%** | **98.3%** | ✅ |
```
...
- [ ] このファイルで扱うテスト対象クラスは3つ以下か？
- [ ] テストケース数は50以下か？
- [ ] ページ数は30ページ以下に収まる見込みか？
- [ ] 1つのモジュール/機能に絞られているか？
